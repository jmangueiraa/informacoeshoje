...
export const Route = createFileRoute('/$slug')({
  loader: async ({ params }) => {
    ...
    try {
      ...
      console.warn('Erro no loader server-side de redirecionamento:', e)
    }

    return { targetUrl: null }
  },
  component: RedirectHandlerPage,
})

function RedirectHandlerPage() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data: link, error }) => {
        if (error || !link) {
          console.error('Link não encontrado no Supabase:', error)
          window.location.replace('/')
          return
        }
